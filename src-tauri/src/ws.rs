...
use tauri::{
    ...
    Manager, Runtime, State, Window, WindowEvent,
};
use tokio::{
    net::TcpStream,
    sync::Mutex,
...
struct WebSocektState(WebSocketWriter, Instant, bool);
...
#[derive(Default)]
struct ConnectionManager(Mutex<HashMap<Id, WebSocektState>>);
...
async fn connect<R: Runtime>(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        manager
            ...
            .lock()
            .await
            .insert(id, WebSocektState(write, time::Instant::now(), false));
        while let Some(message_result) = read.next().await {
            let should_break = matches!(&message_result, Ok(Message::Close(_)));
            if let Ok(Message::Ping(_)) = &message_result {
                if let Some(state) = manager.0.lock().await.get_mut(&id) {
                    let WebSocektState(_, _, ping_ref) = state;
                    *ping_ref = true;
                }
            }
            let response = match message_result {
                ...
                Ok(Message::Frame(_)) => continue, // Raw frames are not handled, just continue.
                Err(e) => {
                    // An error on the stream also means we should break and cleanup.
                    let _ = on_message.send(serde_json::to_value(Error::from(e)).unwrap());
                    break;
                }
            };
            if on_message.send(response).is_err() {
                // Log for debugging purposes
                eprintln!("[websocket-proxy] Tauri client for connection {} disconnected. Closing upstream connection.", id);
        ...
        eprintln!("[websocket-proxy] Cleaning up connection: {}", id);
        let writer_to_close = manager.0.lock().await.remove(&id);
        if let Some(mut state) = writer_to_close {
            let WebSocektState(ref mut writer, _instant, _pong) = state;
...
async fn send(
    ...
    if let Some(state) = manager.0.lock().await.get_mut(&id) {
        let WebSocektState(ref mut write, _instant, ref mut pong) = state;
        write
            .send(match message {
        ...
        PluginBuilder::new("websocket")
            ...
            .setup(|app, _api| {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    let plus = Duration::from_secs(1);

                    loop {
                        tokio::time::sleep(check_interval).await;
                        let conn_manager = app_handle.state::<ConnectionManager>();
                        let connections_to_close = {
                            let mut connections = conn_manager.0.lock().await;
                            let mut ids: Vec<u32> = Vec::new();
                            for (id, state) in connections.iter_mut() {
                                let WebSocektState(_writer, instant_ref, ping_ref) = state;

                                // println!(
                                //     "[Connection {}] is still alive. Elapsed: {:?}",
                                //     id,
                                //     instant_ref.elapsed()
                                // );
                                if instant_ref.elapsed() > check_interval && *ping_ref {
                                    ids.push(id + 0);
                                }
                                if !*ping_ref {
                                    *instant_ref = time::Instant::now() + plus;
                                }
                            }
                            ids
                        };
                        // println!("{:?}", connections_to_close);
                        for id in connections_to_close {
                            let writer_to_close = conn_manager.0.lock().await.remove(&id);
                            // println!("Removing {}", id);
                            if let Some(mut state) = writer_to_close {
                                let WebSocektState(ref mut writer, _instant, _pong) = state;
                                let _ = writer.send(Message::Close(None)).await;
